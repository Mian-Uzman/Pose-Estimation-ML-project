...
def bodypix_mobilenet_config(stride, quant_bytes=4, multiplier=1.0):
    ...
    # quantBytes = 4 corresponding to the non - quantized full - precision checkpoints.
    if quant_bytes == 4:
        base_url = MOBILENET_BASE_URL + 'float/' + \
            multiplier_map[multiplier] + '/'
        model_dir = MOBILENET_MODEL + '_float_' + multiplier_map[multiplier]
    else:
        base_url = MOBILENET_BASE_URL + 'quant' + \
            str(quant_bytes) + '/' + multiplier_map[multiplier] + '/'
        model_dir = MOBILENET_MODEL + '_quant' + \
            str(quant_bytes) + '_' + multiplier_map[multiplier]
